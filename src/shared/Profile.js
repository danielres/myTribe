...
const Profile = ({ handleClickTo, person }) =>
  <div className="entries">
    ...
    <Entry>
      Member since: {person.infos.addedAt}
    </Entry>

    <LinkTo external target={person.infos.fbProfileUrl}>
      <Entry>
        FB profile: {person.infos.fbProfileUrl}
      </Entry>
    </LinkTo>

    <LinkTo external target={person.infos.introUrl}>
      <Entry>
        Intro: {person.infos.introUrl}
      </Entry>
    </LinkTo>

    <LinkTo external target={`mailto:${person.infos.email}`}>
      <Entry>
        Email: {person.infos.email}
      </Entry>
    </LinkTo>

    <Entry>
      Phone: {person.infos.phone}
    </Entry>

    <Entry>
      Address: {person.infos.address}
    </Entry>
  </div>
